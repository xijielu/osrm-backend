...
max_number_of_alternatives           | Unsigned | Maximum number of alternative routes to return. Note: there is no guarantee for a minimum. (default `3`)
max_number_of_alternatives_to_unpack | Unsigned | Maximum number of alternative paths to unpack and run high-quality checks on. Set lower for higher performance but also lower chance to find alternative routes. (default `15`)
